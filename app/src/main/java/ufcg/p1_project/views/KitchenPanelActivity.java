...
public class KitchenPanelActivity extends AppCompatActivity {
    ...
    private KitchenPanelActivity.ViewHolder mViewHolder = new KitchenPanelActivity.ViewHolder();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (int i = 0; i < listaTemp.size(); i++) {
            if (listaTemp.get(i).getNome().equals(zome.getImovelAtual())) {
                ...
                if (reg.getUsersData().get(upl.getLoggedUser()).getListaImoveis().get(i).getZonesList().isEmpty()) {
                    ...
                    pontosVazao = reg.getUsersData().get(upl.getLoggedUser()).getListaImoveis().get(i).getZonesList().get(1).getPontosDeVazao();
                }
        ...
        mViewHolder.pontosVazaoLv = findViewById(R.id.lv_flow_points_kitchen);
